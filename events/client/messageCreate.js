...
module.exports = {
  ...
  run: async (client, message) => {
    ...
    let [premiumGuild, premiumUser] = await require(`@functions/msgCrt/checkPremium.js`)(message);
    ...
    if (message.content.match(mention)) {
      ...
      if (mentionRlBucket.limited && !owner && !admin)
        return client.db.blacklist.set(`${client.user.id}_${message.author.id}`, true);
    ...
    if (command.args && !args.length) {
      ...
      return await message.reply({
        embeds: [new EmbedBuilder().setColor('Red').setDescription(reply)],
      });
    }
    ...
    try {
      if (command.execute) {
        ...
        console.error(`Command ${commandName} does not have an execute method.`);
      }
    } catch (error) {
